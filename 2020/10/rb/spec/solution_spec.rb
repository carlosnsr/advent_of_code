...
RSpec.describe '#count_voltage_differences' do
  context 'given the test input' do
    ...
    it 'the number of 1-volt and 3-volt differences' do
      ...
      expect(count_voltage_differences(enumerator)).to eq([22, 10])
    end
  end

  it 'for array, assume 3-difference between the outlet and the final device' do
    expect(count_voltage_differences([])).to eq([0, 1])
  end

  it 'compares the first value to 0 (the outlet) and then +3 (the device)' do
    expect(count_voltage_differences([1])).to eq([1, 1])
    expect(count_voltage_differences([2])).to eq([0, 1])
    expect(count_voltage_differences([3])).to eq([0, 2])
  end

  it 'can process multi-item sorted arrays' do
    expect(count_voltage_differences([1, 2, 3])).to eq([3, 1])
    expect(count_voltage_differences([3, 6, 9])).to eq([0, 4])
  end
end
